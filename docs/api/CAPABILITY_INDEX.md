...
| **`model.provider.v1`** | Unified external model computation provider, including embedding facts | `cyrene/model/provider/v1/model_provider.proto` + `cyrene/capability/v1/capability_execution.proto` | Typed `Any` payload + CES | **EXPERIMENTAL** | Exchange / Navigator / Echo compatibility seams | `plugins/providers/model-api-connector` | **LEGACY_COMPATIBILITY_SHIM** |
...
| **`memory.provider.v1`** | Legacy external memory compatibility seam; not the embedding computation contract | `plugin/v1/storage.proto` | Storage Payload | **EXPERIMENTAL** | Exchange / Navigator | `plugins/data/memory` (`vector_worker`) | **LEGACY_COMPATIBILITY_SHIM** |
