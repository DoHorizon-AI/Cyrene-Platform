# src Directory Guide | contracts/rust/cy-manifest/src 目录指南
...
| Entry | Responsibility | 一句话职责 |
| --- | --- | --- |
| `canonical.rs` | Rust implementation, contract, or test file. | Rust 实现、契约或测试文件。 |
...
| `manifest/` | Nested source or contract boundary; read its README next. | 嵌套源码或契约边界，下一步阅读其 README。 |
...
# contracts/rust/cy-manifest/src 目录指南
...
| 条目 | 职责 |
| --- | --- |
| `canonical.rs` | Rust 实现、契约或测试文件。 |
| `lib.rs` | Rust 实现、契约或测试文件。 |
| `bin/` | 嵌套源码或契约边界，下一步阅读其 README。 |
| `manifest/` | 嵌套源码或契约边界，下一步阅读其 README。 |
