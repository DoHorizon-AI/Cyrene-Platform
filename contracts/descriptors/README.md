...
module, including imports and source information. The deterministic
`cyrene-core-v1-stable.pb` SHA-256 is recorded in the Core v1 fixture manifest.
Any contract change must regenerate the
...
`cyrene-core-v1.pb` 是 Core v1 module 的已检入 descriptor 基线，包含 imports 和源文件信息。确定性 `cyrene-core-v1-stable.pb` 的 SHA-256 记录在 Core v1 fixture manifest 中。任何契约变更都必须重新生成 descriptor，并在替换基线前通过 Buf breaking check。
