...
class PortableDirectoryManifest:
    ...
    def validate(self) -> None:
        if self.version != PORTABLE_DIRECTORY_MANIFEST_VERSION:
            raise ValueError(
                "portable directory manifest version must be "
                f"{PORTABLE_DIRECTORY_MANIFEST_VERSION}, got {self.version}"
            )
        ...
        if logical_size != self.size_bytes:
            raise ValueError(
                "portable directory logical size mismatch: "
                f"declared {self.size_bytes}, computed {logical_size}"
            )
        ...
        for path in paths:
            ...
            for component in components[:-1]:
                ...
                if candidate in exact_paths:
                    raise ValueError(
                        f"portable directory has a file/directory path conflict at {candidate!r}"
                    )
